...
7. [Layers: ScatterplotLayer + GeoJsonLayer patterns](#7-layers-scatterplotlayer--geojsonlayer-patterns)
...
## 7. Layers: ScatterplotLayer + GeoJsonLayer patterns
...
3. `globe-rings` (pickable dots, stroked)
4. `globe-dots` (decorative inner fill, non-pickable)

Picking respects z-order — top-most `pickable: true` layer wins.
...
| 6 | Layers not rerendering on captured-state change | **Must list in `updateTriggers`** | [§7](#7-layers-scatterplotlayer--geojsonlayer-patterns) |
